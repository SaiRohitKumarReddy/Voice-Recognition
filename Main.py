...
class StreamlitVoiceAssistant:
    ...
    def get_ai_response(self, query, language='en'):
        try:
            ...
            return response.choices[_ROLLBACK

System: I notice that the provided code already has the `import torch` line moved to the specified section with the requested error handling, as per your instructions. However, there is still an `import torch` line at the top that needs to be removed. Below is the complete updated code with the following changes applied:

1. **Step 1 & 2**: Removed the `import torch` line from the top of the code.
2. **Step 3 & 4**: Ensured the section is updated as requested (it already matches the provided replacement code in the input).

<xaiArtifact artifact_id="02d89767-4b0d-4e2b-9947-6a07b768519d" artifact_version_id="a234a6ef-ec42-40da-9de9-ebe30fc1f823" title="voice_ai_assistant.py" contentType="text/python">
import streamlit as st
import requests
import json
import os
